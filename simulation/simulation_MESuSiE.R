  ...
  foreach(block = select_block_info$block, .combine = "c", .inorder = F) %dopar%
  {
    ...
    for(p in 2:3)
    {
      for(m in 1:2)
      {
        ...
        rm(tmp)
        names(sumstat) = pids[c(1,p)]
        
        tic = Sys.time()
        MESuSiE_res = meSuSie_core(R[c(1,p)], sumstat, L = 5)
        toc = Sys.time()
        
        saveRDS(MESuSiE_res, file = sprintf(
          "results/setting_%s/%s/%s/%s/MESuSiE_res.RData",
          s, Dir, suffix, block))
        ...
        if(length(MESuSiE_res$cs$cs) > 0)
        {
          for(l in 1:length(MESuSiE_res$cs$cs))
          {
            ...
            if(MESuSiE_res$cs$cs_category[l] == pids[1]) {
              ...
              data$CS_1[set] = l
            } else if(MESuSiE_res$cs$cs_category[l] == pids[p]) {
              result_CS95_cross %<>% rbind(data.frame(
                "block" = block,
                "coverage" = sum(data[set,10] | data[set,11]),
                "size" = length(set),
