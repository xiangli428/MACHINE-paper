...
Dir = "MACHINE_1kg-gLDSC"
# Dir = "MACHINE_1kg-polyfun"

for(s in 1:3)
{
  gLDSC_var = readRDS(sprintf(
    "gLDSC/gLDSC_results/1kg/setting_%d/gLDSC_var.RData", s))
  for(k in 1:5)
  {
    gLDSC_var[,k] %<>% pmax(max(gLDSC_var[,k]) / 20)
  }
  
  # gLDSC_var = readRDS(sprintf("polyfun_output/1kg/setting_%d/polyfun_var.RData", s))
  ...
  foreach(block = select_block_info$block, .combine = "c", .inorder = F) %dopar%
  {
    ...
    R_eig = readRDS(sprintf("data/LD_1kg/eig_%d.RData", block))
