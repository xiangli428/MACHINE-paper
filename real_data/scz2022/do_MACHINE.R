...
foreach(l = 1:nrow(min_p), .combine = "c", .inorder = F) %dopar%
{
  ...
  data = read_delim(sprintf("data/merge/chr%d/block_%d.txt.gz", i, block), 
...
# CS info
CS_info = foreach(l = which(rowSums(results[,c(13,19)]) > 0),
                  .combine = "rbind") %do%
{
  i = results$chromosome[l]
  block = results$block[l]

  tmp = read.delim(sprintf("%s/chr%d/%d/CS_info.txt", Dir, i, block))
  tmp$chromosome = i
  tmp$block = block
  tmp[,c(6:7,1:5)]
}

write_delim(CS_info, sprintf("results/%s/CS_info.txt", Dir), delim = '\t')
