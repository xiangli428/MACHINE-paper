...
foreach(l = 1:nrow(min_p), .combine = "c", .inorder = F) %dopar%
{
  ...
  write_delim(data.frame("RSID" = data$rsid, "Z" = z), sprintf(
    "%s/%s/chr%d/%d/z.txt", Dir, pid, i, block), delim = '\t')
  write.table(R, sprintf("%s/%s/chr%d/%d/ld.txt", Dir, pid, i, block),
  ...
  system(paste("/home/r9user9/anaconda3/bin/python",
               ...
               "--z", sprintf("%s/%s/chr%d/%d/z.txt", Dir, pid, i, block),
               "--ld", sprintf("%s/%s/chr%d/%d/ld.txt", Dir, pid, i, block),
               "--save", sprintf("%s/%s/chr%d/%d/output", Dir, pid, i, block),
               sprintf("--K %d", K)))
  output = read.delim(sprintf("%s/%s/chr%d/%d/output.rsparsepro.txt", 
                              Dir, pid, i, block))
  ...
  system(sprintf("rm %s/%s/chr%d/%d/z.txt", Dir, pid, i, block))
  system(sprintf("rm %s/%s/chr%d/%d/ld.txt", Dir, pid, i, block))
