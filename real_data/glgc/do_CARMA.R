...
foreach(db = dbs) %do%
{
  ...
  foreach(pheno = phenos[2:4]) %do%
  {
    ...
    {
      pid = pids[k]
      min_p_sub = min_p[min_p[2+k] > 0, c(1,2,2+k,5+k)]
      names(min_p_sub)[3] = "num_variants"
      
      Ls = foreach(l = 1:nrow(min_p_sub), .combine = "c") %dopar%
      {
      ...
      foreach(l = Ls, .combine = "c", .inorder = F) %dopar%
      {
        i = min_p_sub$chromosome[l]
        block = min_p_sub$block[l]
        
        dir.create(sprintf("%s/%s/%s/%s/chr%d/%d", Dir, pheno, db, pid, i, 
                           block), recursive = T)
        ...
        CARMA_results <- CARMA(
          ...
          output.labels = sprintf("%s/%s/%s/%s/chr%d/%d", Dir, pheno, db, 
                                  pid, i, block))
        toc = Sys.time()
        
        saveRDS(CARMA_results, file = sprintf(
          "%s/%s/%s/%s/chr%d/%d/CARMA_results.RData", Dir, pheno, db, pid, i, 
          block))
        ...
        write_delim(data, gzfile(sprintf(
          "%s/%s/%s/%s/chr%d/%d/variant_info.txt.gz", Dir, pheno, db, pid, 
          i, block)), delim = '\t')
...
for(db in dbs)
{
  for(pheno in phenos)
  {
    ...
    num_CS = merge(results[[1]][,c(4,5,10)], results[[2]][,c(4,5,10)],
                   by = c("chromosome","block"), all = T) %>%
      merge(results[[3]][,c(4,5,10)], by = c("chromosome","block"), all = T)
