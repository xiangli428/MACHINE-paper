  ...
  for(pheno in phenos)
  {
    ...
    for(k in 1:3)
    {
      ...
      foreach(l = 1:nrow(min_p_sub), .combine = "c", .inorder = F) %dopar%
      {
        ...
        data = read.delim(gzfile(sprintf("data/%s/%s/%s/chr%d/block_%d.txt.gz", 
        ...
        L = 5
        tic = Sys.time()
        fitted_susie = susie_rss(z, R, n, L = L, refine = T)
        while((length(fitted_susie$sets$cs) == L) & (L < 20))
        {
          L = L + 5
          fitted_susie = susie_rss(z, R, n, L = L, refine = T)
        }
        ...
        saveRDS(fitted_susie, file = sprintf(
          "%s/%s/%s/%s/chr%d/%d/fitted_susie.RData", 
        ...
        data$CS = NA
        n_CS = length(fitted_susie$sets$cs)
        if(n_CS > 0)
        {
          for(s in 1:n_CS)
          {
            ...
            CS_info %<>% rbind(data.frame(
              "CS" = CS_id,
              "CS.size" = length(set),
              "nlog10minp" = max(data$neglog10_pval[set]),
        ...
        result = data.frame(
          ...
          "num_variants" = M,
          "L" = L,
          "converged" = fitted_susie$converged,
...
for(db in dbs)
{
  for(pheno in phenos)
  {
    ...
    num_CS = merge(results[[1]][,c(4,5,12)], results[[2]][,c(4,5,12)],
                   by = c("chromosome","block"), all = T) %>%
      merge(results[[3]][,c(4,5,12)], by = c("chromosome","block"), all = T)
